...
Document version: 1.0.2 | Last updated: July 2026 — Organisation contract; organisation branding; Users: `isClinician`, `isLocum`, `authorisedSignatory`.
...
| Field               | Type           | Description                                              |
| --------------------- | ---------------- | ---------------------------------------------------------- |
| extOrganisationId   | string         | Echo of path parameter                                   |
| contractSigned      | boolean        | `true` if the organisation contract has been signed      |
| contractSignedAt    | string \| null | ISO-8601 timestamp of signing; `null` if not signed      |
...
Mints a short-lived organisation-scoped token used to open the ORB organisation-contract signing iframe. Available **before** the contract is signed (not gated). The token identifies the API-key client and external organisation only (no patient or user claim).
...
| Field        | Type    | Description                                      |
| -------------- | --------- | -------------------------------------------------- |
| accessToken  | string  | JWT for the contract-signing iframe session      |
...
Embedding, `postMessage`, and the in-iframe signing UI are documented under [Organisation contract (clinic)](#organisation-contract-clinic) and in the [ORB Widget Integration Guide](../widget-integration.md).
...
Sets organisation branding used on the **patient-facing NHS linking page** opened from Connect Patient by Email / Connect Patient by Link: organisation logo and optional welcome title/subtitle.

Requires the organisation contract to be signed (`403` **OrganisationContractNotSigned** otherwise).

A successful call returns **204** with no body.
...
| file | file | No | Logo image — PNG, JPEG, JPG or SVG, max **1 MB**. Uploading a new file replaces the previous logo. |
...
Welcome text and logo can be updated independently (send only the fields you want to change).
...
- an **NHS Login** button only (no self-register option).
...
| 415    | Unsupported media type (file not PNG/JPEG/JPG/SVG, content mismatch, or not multipart) |

---

# Users

Users represent people in the EHR who are linked to ORB via the integration. They do not have general login access to the ORB UI.
...
### Required behaviour (`isClinician`)

- **`isClinician` is mandatory** when creating a user via the API.
- **`isClinician` is optional** when updating a user.
- **`isClinician` is always returned** in user list responses.
...
| ----------- | ----------- | ---------- |
| NHS registration / invitation link | The patient has not completed NHS registration | The expiry period is system-configurable and is **7 days by default**, starting when the link is generated. Email and direct-link flows use the same type of token. |
| Data-access / consent link | The patient has completed NHS registration and needs to review access for the organisation | The URL has **no separate expiry period**. It takes the patient to ORB login and requires authentication before access can be reviewed. |
...
The data-access / consent URL does not contain an expiring invitation token, so requesting it again does not renew an expiry period. Reissuing or resending it is still subject to the data-access request rate limit above.
...
| 429 | Rate limit exceeded: the maximum number of connection emails allowed per patient per organisation in 24 hours has been reached. |
...
An NHS registration link expires after the configured invitation period (**7 days by default**). Generating a replacement invalidates the previous registration link and restarts the expiry period. A returned data-access / consent link is an authenticated ORB login URL and has no separate link expiry.
...
| 409 | Conflict — external patient ID already in use by another patient, or request body DOB/surname does not match existing ORB patient record (ORB vs request mismatch). Distinct from 422 (NHS→ORB mismatch, see below). |
| 422    | Returned when the patient's connection status is **DataMismatch** (NHS login revealed a mismatch between ORB and NHS data). Response body includes `connectionStatus` **DataMismatch** with `dobMismatch`/`surnameMismatch`. Distinct from 409 (request body does not match ORB, or external patient ID already in use). |
| 429  | Rate limit exceeded: the maximum number of connection links allowed per patient per organisation in 24 hours has been reached.                          |
...
The response may include **`nhsNumber`**: the patient's NHS number from ORB's NHS Login record (verified via NHS Login). This field is **read-only** for the EHR — it is not supplied on connect or update requests. `nhsNumber` is **omitted or null** when the patient has no NHS Login on record.
...
- The underlying patient account, PatientLogin, and NHS/IM1 data are **retained**. Other organisation links for the same patient are untouched.
...
The ORB widget is a JavaScript component that must be embedded in the EHR's patient record page. Embedding, token handling, and runtime behaviour are documented in the [ORB Widget Integration Guide](../widget-integration.md). The token is passed to the widget via `postMessage` (recommended; see that guide). Do not call `/access-token` from the browser.

Clinician widget Terms and Conditions are required before NHS data is shown in the widget. See [Terms and Conditions](#terms-and-conditions).
...
| 403    | Access denied — user not authorised (including `isClinician: false`), organisation suspended, or patient permissions insufficient (`errorCode`: `NonClinicianAccess` when the user is non-clinical) |
...
| Agreement | Who accepts | Status in this guide |
| ----------- | ------------- | ---------------------- |
| [Clinician widget T&C](#clinician-terms-and-conditions-widget) | Each clinician user, per organisation | Documented below |
| [Organisation contract (clinic)](#organisation-contract-clinic) | Authorised organisation representative (host-controlled) | Documented below |
...
This is the **clinician widget T&C** only (tick-box acceptance in the iframe). It is **not** the organisation-level clinic contract.
...
| T&C content | System-defined wording hosted on ORB as an editable Static Page (alias `widget-terms-and-conditions`), so ORB can update the text without an EHR release |
...
- **Re-acceptance:** once accepted for a given user–organisation pair, the clinician is **not** required to re-accept T&Cs for the same organisation (acceptance is idempotent).

### What ORB records

On acceptance, ORB records that the clinician approved the T&Cs for that user–organisation pair (including when it was accepted), for audit and compliance. Until acceptance is completed, widget IM1/data reads return `403` with `errorCode` **TermsNotAccepted**.

### Hosting and context

- T&C pages / content are **hosted by ORB** (MiPro / ORB sites) and shown inside the third-party EHR iframe (e.g. Semble).
- Clinician and organisation context for acceptance comes from the **widget access token** (scoped user, organisation, patient), not from calling External API T&C endpoints.
- PDF download of the current T&C wording is also served from ORB for the widget session (ORB-internal widget APIs).
...
**EHR integrators:** Acceptance and enforcement are handled **inside the ORB widget** after the EHR passes the access token. The **ORB External API** does not expose endpoints to accept clinician T&C or to query acceptance status. See also the [ORB Widget Integration Guide](../widget-integration.md).

## Organisation contract (clinic)

As part of iFrame-related onboarding, each EHR organisation must complete a separate **organisation-level** ORB contract / Terms & Conditions signing flow before protected API and Widget functionality for that organisation is available.

The signing UI is hosted by ORB and embedded in an iframe controlled by the host EHR (e.g. Semble). Contract content, signing logic, audit logging, PDF generation, and download links are handled on the ORB side. 
...
| Content | Contract wording is hosted by ORB and shown inside the iframe |
| Re-signing | One signature per organisation; once signed, `contract-status` reports `contractSigned: true` |
...
3. When ready, mint `POST /organisations/{extOrganisationId}/contract-access-token`.
4. Open the ORB contract page in an iframe (path `/medical-record-embedded/organisation-contract/sign`) and pass the token via `postMessage` `SET_TOKEN`. Optionally send `SET_SIGNATORY` with `{ first_name, last_name, email }` to pre-fill the form.
5. Signatory reviews the contract, enters full name and email, ticks acceptance, and submits **inside the iframe**. The host does **not** call a separate External API endpoint to record the signature — ORB handles acceptance within the widget.
6. On success, the iframe posts `CONTRACT_SIGNED` with `{ signedContractUrl }`, shows confirmation, and a **Download Signed Contract** button. A signed PDF is also emailed to the address submitted.
7. Host may poll contract-status again or listen for `CONTRACT_SIGNED` / `orb-widget-contract-signed`.

Details of embedding and `postMessage` are in the [ORB Widget Integration Guide](../widget-integration.md).

### After signing

ORB records the signatory name and email, signing time, and a downloadable signed PDF (also emailed to the signatory). Download from the confirmation screen uses a login-free link.
...
| POST | `/organisations/{extOrganisationId}/contract-access-token` | Mint iframe token |

### Blocked until the contract is signed

Until `contractSigned` is true, protected External API and widget operations for that organisation return **403** with `errorCode` **OrganisationContractNotSigned** (see [Common Errors](#common-errors)).
